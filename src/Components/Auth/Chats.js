...
function Chats() {
  ...
  useEffect(() => {
    axios
      .post("https://shattapp-api.onrender.com/user/details", { id: token })
      .then((response) => {
  ...
  useEffect(() => {
    if (chatSearch.length > 0) {
      axios
        .post("https://shattapp-api.onrender.com/users/find", {
          string: chatSearch,
        })
        .then((res) => {
  ...
  function sendMessage() {
    ...
    axios
      .post("https://shattapp-api.onrender.com/chats/update", object)
      .then((res) => {
        setCurrentContact({
          ...currentContact,
          conversation: tempConversations,
        });
      });
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-main">
        <div className="chat-section">
          <span
            className={`contact-icon icon-show`}
            onClick={() => setContactsVisibility(true)}
          >
            <i className="fad fa-users"></i>
          </span>
          <span
            className={`contact-icon close-contacts ${
              isContactsVisible ? "icon-show" : "icon-hide"
            }`}
          ...
          <div
            className={`contacts ${
              isContactsVisible ? "contacts-show" : "contacts-hide"
            }`}
          >
                ...
                return (
                  ...
                  >
                    <div className="contact-properties">
                      <img src={message.profileURL} className="contact-image" />
          ...
          <div
            className={`${
              isRenderChatVisible ? "render-chat-show" : "render-chat-hide"
            } ${isContactsVisible ? "render-chat-small" : "render-chat-full"}`}
          >
            {/* <span className="current-contact-name">
                            Dr Braavosi
                        </span> */}
            <span className="current-contact-name">{currentContact.name}</span>
                      ...
                      <div className="chat-container received">
                        <div className="chat received-chat">
                          <div className="chat-inside">
                            <p className="chat-content">{message.body}</p>
